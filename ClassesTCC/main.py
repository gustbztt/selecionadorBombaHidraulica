...
def final():
    ...
    curva_recalque = CurvaSistema(dados.temperatura, dados.altura_succao, dados.altura_recalque,
                                  dados.diametro_recalque, dados.material, dados.comprimento_recalque)

    dados.dic_recalque = dic_recalque

    NPSHd, hm_succao, rho = curva_succao.run_succao(
    ...
    intersection = CurveIntersection(
        df_hm, hm_sistema, df_NPSH, NPSHd, df_potencia, dados.rho)
    ...
    html_table_blue_light = build_table(
        merged_df, "blue_dark", text_align="center", width="100%", padding='10px'
    )
